...
{ 
  ...
  /* Set Publisher and Rate for F/S--- Sensors */
  else if( strstr((char*)_SensorList.at(_SelectedSensorID).SensorName.c_str(), "F") || 
           strstr((char*)_SensorList.at(_SelectedSensorID).SensorName.c_str(), "S") )
  {
  ...
  // ofstream fout("/home/gianluca/Scrivania/frequenza_sensori/opening_program/log_500hz_"+_SensorList.at(_SelectedSensorID).SensorName+".txt"); //##################################
  // fout<<"t_cycle"<<endl; //##################################
  // ros::Time tstart, tend; //##################################
  // bool wf = false;
  // ros::Time tstart_cycle, tend_cycle; //##################################
  ...
  while (ros::ok())
  {
    ...
    /* Sensor readings for F/S--- Sensors */
    else if( strstr((char*)_SensorList.at(_SelectedSensorID).SensorName.c_str(), "F") ||
             strstr((char*)_SensorList.at(_SelectedSensorID).SensorName.c_str(), "S") )
    {
      // if (wf) tstart = tactile_sensor_data_msg.header.stamp;
      // tstart_read = ros::Time::now(); //##################################
      /* Request sensor data */
      size_t bytes_wrote = _SensorSerial->write("a");
      ...
      for(int k = 0U; k < _SensorList.at(_SelectedSensorID).NumSensingElements; k++)
      {
        _SensorList.at(_SelectedSensorID).LastSensorReadingsRAW.push_back( (float)( dataRaw.at(k*2U)+dataRaw.at((k*2U)+1U)*256U ) * 3.3f /4095.0f );
      ...
      // tend_read = ros::Time::now(); //##################################
      // fout<<(tend_read-tstart_read).toSec()<<", "; //##################################
      ...
      // tend = tactile_sensor_data_raw_msg.header.stamp;
    ...
    loop_rate.sleep();
    // tend = ros::Time::now(); //##################################
    // if (wf) fout<<(tend-tstart).toSec()<<endl; //##################################
  }
  // fout.close(); //##################################
  return 0;
}
